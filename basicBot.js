...
(function () {
    ...
    var botMaintainer = "Benzi / Stereo Sound"
    ...
    var basicBot = {
			...
			if (quizState && quizBand != "" && quizYear != "" && quizCountry != "" && chat.uid != basicBot.room.currentDJID) {
				...
				if (chat.message.match(year) && quizCycle == 1) {
					...
					switch (n1 + n2) {
						...
						case 6:
							quizLastScore *= 2;
							msg = msg + ", and doubled your points: +" + quizLastScore + ".";
							break;
						case 9:
							quizLastScore *= 3;
							msg = msg + ", and tripled your points: +" + quizLastScore + ".";
							break;
						case 12:
							quizLastScore *= 4;
							msg = msg + ", and quadrupled your points: +" + quizLastScore + ".";
							break;
						default:
							msg = msg + ", didn't hit any magic number and scored a total of " + quizLastScore + " points."
							break;
					}
					API.sendChat(msg);
			...
			if (quizState) {
				...
				if (quizBand != "" && quizLastScore != 0) {
					if (quizUsers.length > 0) {
						for (var i = 0; i < quizUsers.length; i++) {
							if (quizUsers[i][0] == quizLastUID) {
								...
								if(quizUsers[i][2] >= parseInt(quizMaxpoints,10)) {
									API.sendChat("@" + quizUsers[i][1] + " You Won! Congrats, you'll be remembered for centuries. Isn't that the best price you can win? ^^");
									quizState = false;
								} else {
									API.sendChat("@" + quizUsers[i][1] + " Points: " + quizLastScore + " / Total Score: " + quizUsers[i][2] + " / Points Remaining: " + (parseInt(quizMaxpoints,10) - parseInt(quizUsers[i][2],10)).toString());
								}
								break;
						...
						quizUsers.push([quizLastUID,basicBot.userUtilities.lookupUser(quizLastUID).username,quizLastScore]);
						API.sendChat("@" + quizUsers[0][1] + " Points: " + quizLastScore + " / Total Score: " + quizUsers[0][2] + " / Points Remaining: " + (parseInt(quizMaxpoints,10) - parseInt(quizUsers[0][2],10)).toString());
					}	
				}
								...
								display: function (results) {
									try {
										...
										if (quizCountry != "" && quizYear != "") {
											...
											API.sendChat("In what year was " + quizBand + " founded?");
										}
									} catch (e) {
        ...
        startup: function () {
            ...
            //socket();
			//temp commented: loadChat(API.sendChat(subChat(basicBot.chat.online, {botname: basicBot.settings.botName, version: basicBot.version})));
            loadChat(null);
        },
        commands: {
								...
								//todo: character replace (ie. of mice & men -> &)
									setTimeout(function() {
										try {
											...
											API.sendChat("/me [@" + chat.un + "] NAME: " + name + " // GENRE: " + genres + " // SIMILAR: " + similar + " " + picture);
										} catch (e) {
											API.sendChat("/me [@" + chat.un + "] Sorry, last.fm didn't find any tags for this band.");
										}
									},100);
								}
						}
